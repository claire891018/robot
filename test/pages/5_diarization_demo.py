...
def _init_state():
    """初始化 session state"""
    ss = st.session_state
    ss.setdefault("diar_events", [])
    ss.setdefault("diar_lock", threading.Lock())
    ss.setdefault("sound_window_len", 5000)
    ss.setdefault("diar_send_q", queue.Queue(maxsize=64))
    ss.setdefault("diar_recv_q", queue.Queue())
    ss.setdefault("diar_ws_thread", None)
    ss.setdefault("diar_ws_running", False)

def on_evt(evt: Dict):
    """接收事件"""
    evt['timestamp'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    with st.session_state.diar_lock:
        st.session_state.diar_events.append(evt)
        ...
        try:
            while True:
                ...
                if kind == "audio":
                    ...
                    await ws.send(json.dumps({"type": "reset_speakers"}))
                elif kind == "end":
...
def render_header():
    ...
    st.caption("即時語音辨識 + 說話人辨識。點擊 START 開始說話。")

def render_events(container):
    """渲染辨識結果"""
    with container.container():
        with st.session_state.diar_lock:
            utterances = [e for e in st.session_state.diar_events 
                          if e.get("type") == "utterance_with_speaker"]
        ...
        if not recent:
            ...
            for evt in recent:
                ...
                if txt:
                    ...
                    color = colors.get(speaker_id, "#808080")
                    
                    st.markdown(
                        f'<div style="padding:0.5rem;border-left:4px solid {color};margin-bottom:0.5rem;">'
                        f'<strong>Speaker {speaker_id}:</strong> {txt}'
                        f'</div>',
                        unsafe_allow_html=True
                    )
...
def main():
    ...
    while True:
        if ctx.state.playing and ctx.audio_receiver:
            # 啟動 WebSocket 連線
            if not st.session_state.diar_ws_running:
                t = threading.Thread(
                    target=ws_worker,
                    args=(
                        st.session_state.diar_send_q,
                        st.session_state.diar_recv_q,
                        ASR_WS_URL
                    ),
                    daemon=True,
                )
                t.start()
                st.session_state.diar_ws_thread = t
                st.session_state.diar_ws_running = True
            ...
            for af in audio_frames:
                ...
                if mono_16k.size > 0:
                    # 送給 WebSocket
                    try:
                        st.session_state.diar_send_q.put_nowait(("audio", mono_16k.tobytes()))
                    except queue.Full:
                        try:
                            _ = st.session_state.diar_send_q.get_nowait()
                        except queue.Empty:
                            pass
                        st.session_state.diar_send_q.put_nowait(("audio", mono_16k.tobytes()))
            ...
            while True:
                try:
                    evt = st.session_state.diar_recv_q.get_nowait()
                    on_evt(evt)
                except queue.Empty:
                    break
            ...
            # 停止
            if st.session_state.diar_ws_running:
                try:
                    st.session_state.diar_send_q.put_nowait(("end", None))
                except Exception:
                    pass
                st.session_state.diar_ws_running = False
                st.session_state.diar_ws_thread = None
            break
    ...
    with cols[1]:
        if st.button("👥 重置說話人"):
            try:
                st.session_state.diar_send_q.put_nowait(("reset", None))
                st.success("已重置說話人資料庫")
            except Exception as e:
                st.error(f"重置失敗: {e}")
